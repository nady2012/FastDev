...
/**
 * <b>Description:</b> XSBitmapUtil provides the entry of interfaces to operate
 * bitmap. <br>
 * <b>Purpose:</b> Invoking interfaces in this class to do some operations on
 * bitmap.
 */
public class XSBitmapUtil {
    private static final String TAG = "XSBitmapUtil";
    ...
    public static Bitmap zoomImg(Context mContext, Bitmap bm, int scaleType) {
        ...
        int maxLength = XSDisplayUtil.dip2px(mContext, IMAGE_LENGHT);
        if (width > height) {
            if (((float) width / height) >= 1.5f) {
                newHeight = XSDisplayUtil.dip2px(mContext, MAX_WIDTH_Y);
                newWidth = XSDisplayUtil.dip2px(mContext, MAX_LENGHT);
            } else {
                newHeight = XSDisplayUtil.dip2px(mContext, NORMAL_WIDTH);
            ...
            if (((float) height / width) >= 1.5f) {
                newWidth = XSDisplayUtil.dip2px(mContext, MAX_HEIGHT_X);
                newHeight = XSDisplayUtil.dip2px(mContext, MAX_LENGHT);
            } else {
                newWidth = XSDisplayUtil.dip2px(mContext, NORMAL_WIDTH);
                newHeight = newWidth * height / width;
                if (newHeight > maxLength) {
                    newHeight = XSDisplayUtil.dip2px(mContext, IMAGE_LENGHT);
                }
            }
        } else {
            newWidth = XSDisplayUtil.dip2px(mContext, NORMAL_WIDTH);
        ...
        if (PRE_IMAGE_TYPE == scaleType) {
            if (newHeight > newWidth) {
                // 减去5为了适配IOS气泡宽度问题
                newWidth = XSDisplayUtil.px2dip(mContext, newWidth) - 5;
            } else {
                newWidth = XSDisplayUtil.px2dip(mContext, newWidth);
            }
            newHeight = XSDisplayUtil.px2dip(mContext, newHeight);
        }

        // if (PRE_IMAGE_TYPE == scaleType) {
        // if (newHeight > newWidth) {
        // //减去5为了适配IOS气泡宽度问题
        // newWidth = newWidth - XSDisplayUtil.dip2px(mContext, 5);
        // }
        // }
        // 计算缩放比例
