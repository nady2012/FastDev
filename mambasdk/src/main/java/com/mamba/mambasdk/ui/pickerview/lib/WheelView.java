...
public class WheelView extends View {
    ...
    /**
     * The Paint outer text.
     */
    Paint paintOuterText;
    /**
     * The Paint center text.
     */
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            textColorCenter = a.getColor(R.styleable.pickerview_pickerview_textColorCenter,
                    textColorCenter);
            dividerColor = a.getColor(R.styleable.pickerview_pickerview_dividerColor, dividerColor);
            textSize = a.getDimensionPixelOffset(R.styleable.pickerview_pickerview_textSize,
                    textSize);
            a.recycle();//回收内存
        }
    ...
    private void remeasure() {//重新测量
        ...
        int halfCircumferenceTemp = (int) (itemHeight * (itemsVisible - 1));
        //整个圆的周长除以PI得到直径，这个直径用作控件的总高度
        int measuredHeightTemp = (int) ((halfCircumferenceTemp * 2) / Math.PI);

        int size = MeasureSpec.getSize(heightMeasureSpec);
        measuredHeight = Math.max(size, measuredHeightTemp);
        int itemsVisibleTemp = (int) ((measuredHeight * Math.PI) / (2f * itemHeight)) + 1;
        if (itemsVisibleTemp > 0) {
            if (itemsVisibleTemp % 2 ==0){
                itemsVisible = itemsVisibleTemp+1;
            }else {
                itemsVisible = itemsVisibleTemp;
            }
